        ...
        <section className="space-y-6 rounded-3xl border border-poncho/10 bg-white/80 p-6 shadow-soft" id="regiones">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {regionHighlights.map(({ region, poiCount, topPoi, nextPoi, topCategories }) => (
              <article
                key={region.id}
                className="flex h-full flex-col gap-5 rounded-2xl border border-poncho/10 bg-white/90 p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-md"
              >
                <div className="space-y-3">
                  ...
                  {topCategories.length > 0 && (
                    <div className="flex flex-wrap gap-2 text-xs font-semibold text-cardon/80">
                      {topCategories.map(category => (
                        <span
                          key={category}
                          className="rounded-full bg-poncho/5 px-3 py-1 text-[11px] uppercase tracking-[0.16em]"
                        >
                          {category}
                        </span>
                      ))}
                      ...
                      {nextPoi && (
                        <div className="rounded-xl bg-white/60 p-3 text-sm text-ink/80">
                          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-cardon/70">
                            {locale === 'es' ? 'También cerca' : 'Also nearby'}
                          </p>
